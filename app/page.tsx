...
const cardCls =
  "overflow-hidden rounded-[1.45rem] border border-slate-200/80 bg-white shadow-sm shadow-slate-900/5";
...
function IconTile({
  ...
  return (
    <span className={`${iconBox} ${className} ${tones[tone]}`}>{children}</span>
  );
}
    ...
    <div className="space-y-2.5">
      {items.map((item, i) => {
        ...
        return (
          <div
            key={item.q}
            className={`rounded-2xl border transition-all ${isOpen ? "border-blue-200 bg-white shadow-sm" : "border-slate-200 bg-slate-50/80"}`}
          >
            <button
              type="button"
            ...
            >
              ...
              <span
                className={`ml-4 flex h-6 w-6 shrink-0 items-center justify-center rounded-xl border transition-colors ${isOpen ? "border-blue-200 bg-blue-50" : "border-slate-200 bg-white"}`}
              >
                {isOpen ? (
      ...
      <div>
        ...
        <div className="space-y-2">
          {[
            ...
            {
              name: "Kurs online / webinar",
              cat: "E-learning",
              pts: 15,
              stripe: "bg-slate-500",
            },
            {
              name: "Kurs stacjonarny NIL",
              cat: "Kurs",
              pts: 6,
            ...
            >
              <div
                className={`absolute inset-y-2 left-0 w-1.5 rounded-r-full ${e.stripe}`}
              />
...
function PhotoCard({
  ...
  return (
    <div
      className={`group relative overflow-hidden rounded-[1.45rem] border border-slate-200 bg-slate-900 shadow-sm ${className}`}
    >
      <Image
        src={src}
        alt={alt}
      ...
      <div className="absolute bottom-0 left-0 right-0 p-6 text-white lg:p-7">
        ...
        <h3 className="mt-1.5 max-w-2xl text-xl font-bold leading-tight lg:text-2xl">
          {title}
        </h3>
        {text ? (
      ...
      <div className="grid gap-0 lg:grid-cols-[0.9fr_1.1fr]">
        <div className="relative overflow-hidden border-b border-slate-100 bg-gradient-to-br from-blue-600 via-blue-600 to-blue-700 p-7 text-white lg:border-b-0 lg:border-r lg:p-8">
          ...
          <div className="relative">
            <p className="mb-2 text-[11px] font-bold uppercase tracking-[0.22em] text-blue-100">
              Korzyści
            </p>
            <h2 className="max-w-md text-4xl font-black tracking-tight lg:text-5xl">
              Od szkolenia do porządku. Bez Excela.
            </h2>
            <p className="mt-4 max-w-md text-base leading-relaxed text-blue-50">
              Po szkoleniu dodajesz certyfikat z telefonu. CRPE przypina go do
              aktywności, liczy punkty i pokazuje, czego jeszcze brakuje.
            </p>
            <div className="mt-7 grid gap-3">
              {[
                "Robisz zdjęcie certyfikatu",
                "CRPE pamięta za Ciebie",
                "Widzisz status punktów od razu",
              ].map((item) => (
                <div
                  key={item}
                  className="flex items-center gap-3 rounded-2xl bg-white/12 px-4 py-3 ring-1 ring-white/20 backdrop-blur"
                >
            ...
            <Link
              href="/login"
              className="mt-7 inline-flex items-center gap-2 rounded-xl bg-white px-5 py-3 text-sm font-bold text-blue-700 shadow-lg shadow-blue-950/20 transition hover:-translate-y-0.5 hover:bg-blue-50"
            >
          ...
          <div className="relative rounded-[1.4rem] border border-slate-200 bg-white p-5 shadow-sm shadow-slate-900/5">
            <div className="mb-5 flex items-center justify-between gap-4">
              <div>
                ...
                <p className="text-xs text-slate-500">
                  szkolenie → certyfikat → raport → porządek
                </p>
              </div>
            ...
            <div className="relative grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {flow.map(({ icon: Icon, title, text, tone }, i) => (
                <div
                  key={title}
                  className="relative rounded-2xl border border-slate-200 bg-white p-4 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-blue-200 hover:shadow-lg hover:shadow-blue-950/5"
                >
                  <div className="mb-3 flex items-center gap-3">
                    <IconTile
                      tone={tone}
                      className={`h-12 w-12 ${i === 1 ? "animate-pulse" : ""}`}
                    >
                  ...
                  {i < flow.length - 1 ? (
                    <ArrowRight className="absolute -right-3 top-1/2 z-10 hidden h-5 w-5 -translate-y-1/2 text-blue-500 lg:block" />
                  ) : null}
                </div>
              ))}
            </div>
          ...
          <div className="relative mt-4 grid gap-3 sm:grid-cols-3">
            <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm shadow-slate-900/5">
              ...
              <p className="mt-1 text-xs text-slate-600">
                wpis i certyfikat od razu po szkoleniu
              </p>
            </div>
            <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm shadow-slate-900/5">
              <p className="text-sm font-black text-slate-950">
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm shadow-slate-900/5">
              ...
              <p className="mt-1 text-xs text-slate-600">
                dokumenty masz w jednym widoku
              </p>
            </div>
          </div>
          <div className="relative mt-4 overflow-hidden rounded-2xl border border-emerald-200 bg-emerald-50 p-4">
            <div className="flex items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                <IconTile tone="emerald" className="h-11 w-11">
                  <CheckCircle2 className="h-5 w-5" />
                </IconTile>
                <div>
                  <p className="text-sm font-black text-slate-950">
                    Efekt: wiesz, że temat jest ogarnięty
                  </p>
                  <p className="text-xs text-slate-600">
                    aktywności, certyfikaty, punkty i braki w jednym miejscu
                  </p>
                </div>
              </div>
              <span className="hidden rounded-full bg-white px-3 py-1 text-xs font-bold text-emerald-700 ring-1 ring-emerald-200 sm:inline-flex">
                bez chaosu
              </span>
            </div>
          </div>
        ...
        <div className="grid gap-0 lg:grid-cols-[0.85fr_1.15fr]">
          <div className="border-b border-blue-100 p-6 lg:border-b-0 lg:border-r lg:p-8">
            ...
            <h2 className="text-2xl font-bold tracking-tight text-slate-950">
              Nie przegap brakujących dokumentów
            </h2>
            <p className="mt-3 text-sm leading-relaxed text-slate-600">
              CRPE może pomagać nie tylko w przechowywaniu certyfikatów, ale też
              w pilnowaniu braków i terminów. Dzięki temu łatwiej przygotować
              się do raportu bez nerwowego sprawdzania wszystkiego na końcu
              okresu.
            </p>
          </div>
            ...
            {items.map((item, i) => (
              ...
              >
                <div className="relative mb-3 w-fit">
                  {i === 2 ? (
                    <span className="absolute inset-0 rounded-xl bg-amber-300/40 animate-ping" />
                  ) : null}
                  <IconTile
                    tone="amber"
                    className={`relative h-11 w-11 ${i === 2 ? "animate-bounce" : ""}`}
                  >
...
function HowItWorks() {
      ...
      preview: (
        <div className="space-y-1.5">
          {[
            ...
            <div
              key={x.l}
              className={`flex items-center justify-between rounded-lg px-3 py-2 text-sm ${x.a ? "bg-blue-600 text-white" : "bg-slate-100 text-slate-600"}`}
            >
              <span className="font-medium">{x.l}</span>
              <span
                className={`text-xs ${x.a ? "text-blue-100" : "text-slate-400"}`}
              >
                {x.p}
              </span>
            </div>
      ...
      preview: (
        <div>
          <div className="mb-2 flex items-center gap-3 rounded-lg border border-amber-200 bg-white p-2.5">
            <IconTile tone="amber" className="h-9 w-9">
          ...
          <div className="mb-2 grid grid-cols-2 gap-2">
            <div className="rounded-lg border border-amber-100 bg-white px-2.5 py-1.5 text-xs text-slate-500">
              2026
            </div>
            <div className="rounded-lg border border-amber-100 bg-white px-2.5 py-1.5 text-xs font-semibold text-amber-700">
              Konferencja
            </div>
          </div>
        ...
        <div>
          <div className="mb-2 rounded-xl border border-emerald-100 bg-emerald-50 p-3">
            <div className="flex items-center gap-2">
              <IconTile tone="emerald" className="h-8 w-8">
          ...
          <div className="mt-2 grid grid-cols-3 gap-1.5">
            {[
              ...
              <div
                key={s.l}
                className="rounded-lg border border-slate-100 bg-white py-1.5 text-center"
              >
  ...
  return (
    <div
      ...
      className={`${cardCls} scroll-mt-32`}
    >
          ...
          <div>
            <div className="flex flex-wrap items-center gap-3">
              <h2 className="text-xl font-bold text-slate-900 lg:text-2xl">
                Zobacz, jak CRPE prowadzi Cię krok po kroku
              </h2>
            ...
            <p className="mt-1 text-sm text-slate-500">
              Od wyboru zawodu, przez dodanie certyfikatu, aż po jasny status
              punktów — bez Excela i zgadywania.
            </p>
          </div>
      ...
      <div className="hidden md:block">
        <div className="relative border-b border-slate-100 bg-slate-50/60 px-8 py-6">
          ...
          <div
            className="absolute left-[17%] top-1/2 h-1 -translate-y-1/2 rounded-full bg-blue-600 transition-all duration-700"
            style={{ width: `${activeStep * 33}%` }}
          >
          ...
          <div className="relative grid grid-cols-3 gap-6">
            {steps.map((step, i) => {
              ...
              return (
                ...
                >
                  <span
                    className={`relative flex h-16 w-16 items-center justify-center rounded-2xl border bg-white shadow-sm transition-all duration-300 ${isActive ? `${c.border} scale-110 shadow-lg ring-4 ring-current/10` : "border-slate-200 group-hover:border-blue-200"}`}
                  >
                    <span
                      className={`absolute -left-2 -top-2 flex h-7 w-7 items-center justify-center rounded-full text-xs font-bold text-white ${c.num}`}
                    >
                      {step.n}
                    </span>
                    <Icon
                      className={`h-7 w-7 transition-all ${isActive ? `${c.text} scale-110` : "text-slate-500"}`}
                      strokeWidth={1.9}
                    />
                  </span>
                  <div>
          ...
          {steps.map((step, i) => {
            ...
            return (
              <button
                ...
                onMouseEnter={() => setActiveStep(i)}
                onClick={() => setActiveStep(i)}
                className={`flex min-h-[255px] flex-col border-r border-slate-100 p-6 text-left transition-all duration-300 last:border-r-0 focus:outline-none focus-visible:ring-0 lg:p-7 ${isActive ? "bg-white" : "bg-slate-50/30 hover:bg-white"}`}
              >
                <div
                  className={`mb-4 h-1.5 w-12 rounded-full transition-all ${isActive ? c.bg : "bg-slate-200"}`}
                />
                <p className="text-sm leading-relaxed text-slate-500">
                  {step.desc}
                </p>
                <div
                  className={`mt-auto rounded-2xl border p-3 transition-all duration-300 ${isActive ? `${c.border} ${c.soft} shadow-sm` : "border-slate-100 bg-white/80 opacity-75"}`}
                >
                  {step.preview}
                </div>
              </button>
        ...
        {steps.map((step, i) => {
          ...
          return (
            <div key={step.n} className="p-5">
              <div className="mb-3 flex items-center gap-3">
                <span
                  className={`relative flex h-12 w-12 shrink-0 items-center justify-center rounded-xl border ${c.border} bg-white shadow-sm`}
                >
                  <span
                    className={`absolute -left-2 -top-2 flex h-6 w-6 items-center justify-center rounded-full text-[11px] font-bold text-white ${c.num}`}
                  >
                    {step.n}
                  </span>
              ...
              <div className={`rounded-2xl border p-3 ${c.border} ${c.soft}`}>
                {step.preview}
              </div>
            </div>
          );
...
export default function Page() {
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      const { data: profile } = await supabase
        .from("profiles")
        .select(
          "user_id, profession, period_start, period_end, required_points",
        )
  ...
  const navBase =
    "group inline-flex shrink-0 items-center gap-2 rounded-xl px-4 py-2 text-sm font-medium text-slate-600 transition-all duration-200 hover:scale-[1.03] hover:bg-slate-50 hover:text-blue-700";
  const navActive =
    "group inline-flex shrink-0 scale-[1.03] items-center gap-2 rounded-xl bg-blue-600 px-4 py-2 text-sm font-semibold text-white shadow-sm shadow-blue-600/20 transition-all duration-200";
  const FAQ = [
    {
    ...
    <div className="min-h-screen bg-slate-50 pb-8">
              ...
              {navItems.map(({ id, label }) => {
                ...
                return (
                  <button
                    ...
                    className={isActive ? navActive : navBase}
                  >
                    <span
                      className={`h-1.5 w-1.5 rounded-full ${isActive ? "bg-white" : "bg-slate-300 group-hover:bg-blue-400"}`}
                    />
                    {label}
                  </button>
                );
      ...
      <section className="bg-[radial-gradient(circle_at_top_left,#dbeafe_0,transparent_34%),linear-gradient(180deg,#eff6ff_0%,#f8fafc_100%)] py-7 lg:py-10">
        <div className={pageWrap}>
          <div
            id="hero"
            className="scroll-mt-32 overflow-hidden rounded-[2rem] border border-slate-200/80 bg-white shadow-xl shadow-slate-900/5"
          >
            <div className="relative grid grid-cols-1 gap-8 p-6 sm:p-8 lg:grid-cols-[2fr_1fr] lg:p-10 xl:p-12">
              ...
              <div className="relative z-10 flex flex-col justify-center">
                ...
                <p className="mt-5 max-w-2xl text-lg leading-relaxed text-slate-600">
                  ...
                  w aktualnym okresie rozliczeniowym. Prosto. Spokojnie. Bez
                  Excela.
                </p>
                  ...
                  {[
                    {
                      ...
                      d: "Wpisy i certyfikaty zawsze pod ręką.",
                      tone: "blue" as const,
                    },
                    {
                      icon: BarChart3,
                      t: "Jasny status punktów",
                      d: "Wiesz ile masz i czego brakuje.",
                      tone: "amber" as const,
                    },
                    {
                      icon: ShieldCheck,
                      t: "Bezpieczne dane",
                      d: "Tylko Ty masz dostęp. Dane w UE.",
                      tone: "emerald" as const,
                    },
                    {
                      icon: Sparkles,
                      t: "Start za darmo",
                      d: "Podstawowe funkcje bezpłatnie. Bez karty.",
                    ...
                    return (
                      <div
                        key={b.t}
                        className="flex items-start gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3.5 shadow-sm shadow-slate-900/5"
                      >
                        <IconTile tone={b.tone} className="mt-0.5 h-10 w-10">
                ...
                <div className="mt-8 flex flex-wrap gap-3">
                  <Link
                    href="/login"
                    className="inline-flex items-center gap-1.5 rounded-xl bg-blue-600 px-6 py-3 text-sm font-semibold text-white shadow-lg shadow-blue-600/20 transition hover:bg-blue-700 active:scale-95"
                  >
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-6 py-3 text-sm font-semibold text-slate-700 shadow-sm transition hover:bg-slate-50 active:scale-95"
                  >
                    Jak to działa
                  </button>
                </div>
                <p className="mt-3 text-xs text-slate-500">
                  Bez instalacji • 30 sekund do pierwszego wpisu • działa na
                  telefonie
                </p>
              </div>
              <div className="relative z-10 min-h-[380px] overflow-hidden rounded-[1.6rem] border border-slate-200 bg-slate-900 shadow-xl shadow-slate-950/10 lg:min-h-[500px]">
                ...
                <div className="absolute bottom-0 left-0 right-0 p-5 text-white">
                  ...
                  <p className="mt-1.5 text-sm leading-relaxed text-white/75">
                    Dodajesz wpisy i dokumenty na bieżąco.
                  </p>
                </div>
              </div>
            </div>
      ...
      <section className="bg-slate-50 py-7 lg:py-9">
        <div className={`${pageWrap} space-y-6`}>
          <div className="grid gap-6 lg:grid-cols-[2fr_1fr]">
            ...
            />
            <div className="flex min-h-[390px] items-stretch rounded-[1.45rem] bg-gradient-to-br from-white to-slate-50 p-4 lg:min-h-[455px]">
              <ProductPreview />
            </div>
          </div>
          ...
          <div id="dla-kogo" className="grid scroll-mt-32 gap-6 lg:grid-cols-2">
            <div className={cardCls}>
              <div className="border-b border-slate-100 px-6 py-5 lg:px-8">
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Dla zawodów medycznych zobowiązanych do zbierania punktów
                  edukacyjnych. Lekarze, pielęgniarki, fizjoterapeuci i inni
                  specjaliści, którzy chcą mieć porządek bez Excela.
                </p>
              </div>
            ...
            <div className={cardCls}>
              <div className="border-b border-slate-100 px-6 py-5 lg:px-8">
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Porządek i jasny status. Bez komplikacji.
                </p>
              </div>
              <div className="p-5 lg:p-7">
          ...
          <div className="grid gap-6 lg:grid-cols-[2fr_1fr]">
            ...
            <div className={cardCls}>
              <div className="p-6 lg:p-8">
                ...
                <Quote
                  className="mb-3 h-6 w-6 text-blue-200"
                  strokeWidth={1.5}
                />
                <p className="text-base leading-relaxed text-slate-700">
                  &quot;Wcześniej trzymałam wszystko w Excelu i modliłam się,
                  żeby nie zgubić certyfikatów. Teraz dodaję wpis od razu po
                  szkoleniu — z telefonu.&quot;
                </p>
