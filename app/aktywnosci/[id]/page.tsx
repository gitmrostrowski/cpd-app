...
export default async function ActivityDetailsPage({
  ...
  if (loadError) {
    return (
      <div className="mx-auto max-w-3xl p-6">
        ...
        <Link className="mt-4 inline-block underline" href="/aktywnosci">
          Wróć
        </Link>
      </div>
    );
  ...
  async function onUpload(formData: FormData): Promise<void> {
    ...
    if (!res.ok) {
      redirect(
        `/aktywnosci/${activityId}?err=${encodeURIComponent(res.error)}#certificate`
      );
    }

    redirect(`/aktywnosci/${activityId}#certificate`);
  }
  ...
  async function onDelete(formData: FormData): Promise<void> {
    ...
    if (!res.ok) {
      redirect(`/aktywnosci/${id}?err=${encodeURIComponent(res.error)}#certificate`);
    }

    redirect(`/aktywnosci/${id}#certificate`);
  }
  ...
  return (
    <div className="mx-auto max-w-3xl p-6 space-y-6">
      <div className="flex items-start justify-between gap-4">
        ...
        <Link href="/aktywnosci" className="rounded border px-3 py-2 text-sm hover:bg-gray-50">
          ← Aktywności
        </Link>
      </div>
