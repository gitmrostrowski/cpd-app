...
export default async function ActivityDetailsPage({ params }: { params: { id: string } }) {
  ...
  if (error) {
    return (
      <div className="mx-auto max-w-3xl p-6">
        ...
        <Link className="mt-4 inline-block underline" href="/activities">Wróć</Link>
      </div>
    );
  }
  ...
  if (activity.certificate_path) {
    const { data } = await supabase.storage.from(BUCKET).createSignedUrl(activity.certificate_path, 60 * 60);
  ...
  async function onDelete() {
    "use server";
    return deleteCertificate(params.id);
  }
    ...
    <div className="mx-auto max-w-3xl p-6 space-y-6">
      ...
      <div id="certificate" className="rounded border p-4 space-y-3">
        <div className="flex items-center justify-between gap-4">
          ...
          <span className={`text-xs rounded-full px-2 py-1 ${hasCert ? "bg-green-50 text-green-700" : "bg-gray-100 text-gray-700"}`}>
            {hasCert ? "📎 Dodany" : "Brak"}
          </span>
        </div>
        ...
        {hasCert ? (
          <div className="space-y-3">
            <div className="text-sm text-gray-700">
              Plik: <span className="font-medium">{activity.certificate_name ?? "certyfikat"}</span>
              {activity.certificate_size ? <span className="text-gray-500"> · {(activity.certificate_size / 1024 / 1024).toFixed(2)} MB</span> : null}
            </div>

            <div className="flex flex-wrap items-center gap-2">
              {signedUrl ? (
                <a href={signedUrl} target="_blank" rel="noreferrer" className="rounded border px-3 py-2 text-sm hover:bg-gray-50">
                  Podgląd
                </a>
              ) : (
              ...
              <form action={onDelete}>
                <button type="submit" className="rounded border px-3 py-2 text-sm hover:bg-gray-50">Usuń</button>
              </form>
            </div>

            <div className="pt-3 border-t">
              <p className="text-sm text-gray-600 mb-2">Podmiana pliku: wgraj nowy (stary zostanie usunięty).</p>
              <form action={onUpload} className="flex flex-col gap-2">
                <input name="file" type="file" accept="application/pdf,image/*" required />
                <button type="submit" className="rounded bg-black text-white px-4 py-2 text-sm hover:opacity-90">
                  Wgraj nowy
                </button>
              </form>
            </div>
