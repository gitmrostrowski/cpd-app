...
export default function AdminTrainingsPage() {
  ...
  async function load() {
    ...
    setErr(null);
    try {
      const url = new URL("/api/admin/trainings", window.location.origin);
  ...
  async function patchTraining(
    id: string,
    patch: Partial<TrainingRow> & { status?: TrainingStatus; reject_reason?: string | null }
  ) {
    ...
    const updated: TrainingRow = json.training;
  ...
  async function saveEdit() {
    ...
    setErr(null);
    try {
      const updated = await patchTraining(edit.id, {
        title: edit.title,
  ...
  async function quickApprove(row: TrainingRow) {
    setErr(null);
    try {
      const updated = await patchTraining(row.id, { status: "approved", reject_reason: null });
      if (status !== "all" && updated.status !== status) load();
    } catch (e: any) {
      setErr(e?.message || "Błąd");
    }
  ...
  async function quickReject(row: TrainingRow) {
    const reason = window.prompt("Powód odrzucenia (opcjonalnie):", row.reject_reason || "");
    if (reason === null) return;

    setErr(null);
    try {
      const updated = await patchTraining(row.id, {
        status: "rejected",
  ...
  async function quickPending(row: TrainingRow) {
    setErr(null);
    try {
      const updated = await patchTraining(row.id, { status: "pending", reject_reason: null });
      if (status !== "all" && updated.status !== status) load();
    } catch (e: any) {
      setErr(e?.message || "Błąd");
    }
    ...
    <div className="mx-auto w-full max-w-6xl px-4 py-8">
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-semibold text-slate-900">Weryfikacja i edycja szkoleń</h1>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="flex items-center justify-between border-b border-slate-100 px-4 py-3">
          ...
          <div className="text-xs text-slate-500">Kliknij „Edytuj”, żeby uzupełnić punkty, link, opis itd.</div>
        </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setEdit({ ...edit, points: v === "" ? null : Number(v) });
                  }}
                />
              </div>
